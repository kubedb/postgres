...
DOCKERFILE_PROD  = hack/docker/pg-operator/Dockerfile.in
DOCKERFILE_DBG   = hack/docker/pg-operator/Dockerfile.dbg
...
install:
	@cd ../installer; \
	APPSCODE_ENV=dev KUBEDB_OPERATOR_TAG=$(TAG) KUBEDB_CATALOG=postgres ./deploy/kubedb.sh --operator-name=$(BIN) --docker-registry=$(REGISTRY) --image-pull-secret=$(REGISTRY_SECRET)
