...
func (c *Controller) ValidateSnapshot(snapshot *tapi.Snapshot) error {
	...
	if err := docker.CheckDockerImageVersion(ImagePostgres, c.option.PostgresUtilTag); err != nil {
		return fmt.Errorf(`Image %v:%v not found`, ImagePostgres, c.option.PostgresUtilTag)
	}
