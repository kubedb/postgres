...
func (c *Reconciler) ensureStatefulSet(
	...
	statefulSet, vt, err := app_util.CreateOrPatchStatefulSet(
		...
		func(in *apps.StatefulSet) *apps.StatefulSet {
			...
			in.Spec.Template.Spec.Containers[0].ImagePullPolicy = core.PullAlways
			in.Spec.Template.Spec.Containers[1].ImagePullPolicy = core.PullAlways
			in.Spec.Template.Spec.InitContainers[0].ImagePullPolicy = core.PullAlways
			return in
		},
		metav1.PatchOptions{},
	)
